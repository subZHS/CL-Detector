...
public class EGroumBuilder {
	...
	private static Set<String> buildedJarSet = new LinkedHashSet<>();
	...
	public void buildJar(String jarFilePath) {
		if(jarFilePath == null || buildedJarSet.contains(jarFilePath)){
			return;
		}
		buildedJarSet.add(jarFilePath);
		try {
	...
	private ArrayList<EGroumGraph> buildGroums(TypeDeclaration type, String path, String prefix) {
		ArrayList<EGroumGraph> groums = new ArrayList<>();
		//TODO (myCode)改良原方法
		ASTVisitor methodDeclarationVisitor = new ASTVisitor() {
			@Override
...
//			groums.addAll(buildGroums(inner, path, prefix + type.getName().getIdentifier() + "."));
	...
	public EGroumGraph buildGroum(MethodDeclaration method, String filepath, String name) {
		...
		//TODO (myCode)log current method whose Groum is building
