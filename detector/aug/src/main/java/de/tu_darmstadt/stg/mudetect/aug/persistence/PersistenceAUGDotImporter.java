...
public class PersistenceAUGDotImporter extends DOTImporter<Node, Edge> {
    ...
    private static Node createNode(String label, Map<String, String> attributes) {
        ...
        if (nodeType == SuperMethodCallNode.class) {
            return new SuperMethodCallNode(attributes.get(DECLARING_TYPE), attributes.get(METHOD_SIGNATURE), getSourceLineNumber(attributes));
        }
        ...
        if (nodeType == VariableNode.class) {
            VariableNode node = new VariableNode(attributes.get(DATA_TYPE), attributes.get(DATA_NAME));
            node.setParamIndex(attributes.get(PARAM_INDEX));
            return node;
        }
