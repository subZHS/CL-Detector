    ...
    private static APIUsagePattern toAUGPattern(Pattern pattern) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            if (node instanceof DataNode) {
                LinkedHashSet<DataNode> equivalentNodes = new LinkedHashSet<>();
                //TODO (myCode)把representative放最前
                equivalentNodes.add((DataNode)node);
                List<Fragment> tmpList = new ArrayList<>(pattern.getFragments());
                tmpList.remove(f);
                for (Fragment fragment : tmpList) {
